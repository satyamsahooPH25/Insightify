...
class Column_Cleaner:
    ...
    def start_imputer(self,a,start_index):
        for i in a:
            for j in range(start_index):
                a[i].insert(0,np.nan)
    def word_complete(self,val_dict):
        for i in range(1,len(val_dict)-1):
            pres = list(val_dict[i-1].values())[0]
            post = list(val_dict[i+1].values())[0]
            if pres=='A'and list(val_dict[i].keys())[0]==' ' and post=='A' :
                key_name = list(val_dict[i-1].keys())[0]+' '+list(val_dict[i+1].keys())[0]
                val_name = 'A'
                for u in range(-1,2):
                    try:
                        val_dict.pop(i+u)
                        val_dict.insert(i+u,{key_name:val_name})
                    except:
                        pass
        return val_dict
    ...
    def collimator(self,column_keys,sep):
        ...
        for num in to_delete:
            try:
                ...
                pass
    def create_nan_dataframe(self,num_rows, column_name):
        nan_series = pd.Series([np.nan] * num_rows)
        df = pd.DataFrame(nan_series, columns=[column_name])
        return df
    def Clean_Column(self):
        ...
        for i in data:
            ...
            if(flag==0):
                val_list = self.word_complete(val_list)
        ...
        new_column_keys={}
        for ind,name in enumerate(list(column_keys.keys())):
            curr_name = self.df.columns[self.col_ind]+str(ind)
            old_values = column_keys[name]
            new_column_keys[curr_name]=old_values
        if pd.DataFrame(new_column_keys).empty:
            return self.create_nan_dataframe((self.end-self.start),self.df.columns[self.col_ind]),None
        return pd.DataFrame(new_column_keys),classified_col_values

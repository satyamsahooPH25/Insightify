...
class Augmenter:
    def __init__(self,df,output_excel):
        ...
        self.output_df_prompts=list(self.output_df[self.output_df.columns[1]])
        self.output_df_categories=list(self.output_df[self.output_df.columns[0]])
        if "ID" not in self.output_df.columns:self.output_df["ID"]=[]
    ...
    def synonym(self):
        ...
        for prompt in self.prompt_category_dictionary:
            if prompt in self.output_df_prompts and self.output_df_ID[self.output_df_prompts.index(prompt)]=="synonym":
              ...
              continue
            self.output_df_prompts.extend(prompt) 
            ...
            for aug_prompt in augmented_prompts:
                if(aug_prompt not in self.prompts):
                    self.output_df_prompts.extend(aug_prompt) 
                    self.output_df_categories.append(self.prompt_category_dictionary[prompt])
                    self.output_df_ID.append("synonym")
    ...
    def Contextual_Aug(self):
         for prompt in self.prompt_category_dictionary :
            ...
            for i in range(5):
                if contextual_aug.augment(augmented_prompts[-1]) not in augmented_prompts:
                    augmented_prompts.append(contextual_aug.augment(augmented_prompts[-1]))
            for aug_prompt in augmented_prompts:
                if(aug_prompt not in self.prompts):
                    self.output_df_prompts.extend(aug_prompt) 
                    self.output_df_categories.append(self.prompt_category_dictionary[prompt])
    ...
    def dict2Excel(self):
        ...
        combined_df = pd.concat([existing_df, pd.DataFrame(list(zip(self.output_df_categories,self.output_df_prompts,self.output_df_ID)),columns=["Type","Question"])], ignore_index=True)
